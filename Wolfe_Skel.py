...
def Gradient_V(Oracle,x0):
    ...
    while norm(gradient) > epsilon and compteur < N_it :
        ...
        alpha = Wolfe(1,x,  -gradient  ,Oracle)[0]
    ...
    if compteur>=N_it : print("Gradient_V, k saturated")
...
def BFGS(Oracle,x0):
    ...
    x_k_1 = x0
    g_k_1 = Oracle(x_k_1,3) # gradient à l'étape k-1
    
    alpha = Wolfe(1,x0,-g_k_1,Oracle)[0]
    x_k = x_k_1 - alpha*g_k_1
    ...
    while np.linalg.norm(g_k) >= epsilon and k < 200 :
        
        delta_x = x_k - x_k_1
        delta_g = g_k - g_k_1
        
        #print("d_x: ",delta_x," , d_g: ",delta_g)
        # A_xg = W
        # A_gx = W
        # A_xx = W
        ...
        # for i in range(n):
        #     for j in range(n):
        #         A_xg[i,j] = delta_x[i]*delta_g[j] / np.vdot(delta_x,delta_g)
        #         A_xg[i,j] = delta_g[i]*delta_x[j] / np.vdot(delta_x,delta_g)
        #         A_xg[i,j] = delta_x[i]*delta_x[j] / np.vdot(delta_x,delta_g)
        # W = np.dot((np.eye(n)- A_xg),np.dot(W,((np.eye(n)- A_gx)))) + A_xx 
        ...
        coef_gauche = np.eye(n) - (np.dot(delta_x,np.transpose(delta_g)))/(np.vdot(np.transpose(delta_g),delta_x))
        coef_droite = np.eye(n) - (np.dot(delta_g,np.transpose(delta_x)))/(np.vdot(np.transpose(delta_g),delta_x))
        coef_isole = (np.dot(delta_x,np.transpose(delta_x)))/(np.vdot(np.transpose(delta_g),delta_x))
        W = np.dot(np.dot(coef_gauche,W),coef_droite) + coef_isole
                

        alpha = Wolfe(1, x_k ,  -np.dot(W,g_k)  ,Oracle)[0]
        
        x_k_1 = x_k
        g_k_1 = g_k
        
        x_k = x_k - alpha* (np.dot(W,g_k))
        g_k = Oracle(x_k,3)
